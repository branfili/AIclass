...
def test():
    ...
    for s in smoothing:
        for lt in logtransform[s]:
            ...
            evaluateClassifier(trainPredictions, trainLabels, 'train', classifier.k, classifier.logTransform)
            staticOutputCheck(train_path, s, lt, classifier.posteriors[20:25])

            # evaluate on test set
            testPredictions = classifier.predict(testData)
            evaluateClassifier(testPredictions, testLabels, 'test', classifier.k, classifier.logTransform)
...
def evaluateClassifier(predictions, groundtruth, dataset, k, logTransform):
    accuracy =  [predictions[i] == groundtruth[i] for i in range(len(groundtruth))].count(True)
    print "Performance on %s set for k=%f, log=%s: (%.1f%%)" % (dataset, k, logTransform, 100.0*accuracy/len(groundtruth))
